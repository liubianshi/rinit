# This Makefile is for the RInit extension to perl.
#
...
#     ABSTRACT => q[A modern R project scaffolding tool written in Perl.]
#     AUTHOR => [q[Your Name <your.email@example.com>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => { ExtUtils::MakeMaker=>q[0] }
#     EXE_FILES => [q[bin/rinit]]
#     NAME => q[RInit]
#     PREREQ_PM => { Cwd=>q[0], File::Basename=>q[0], File::Path=>q[0], File::Spec=>q[0], Getopt::Long=>q[0], Pod::Usage=>q[0], Term::ANSIColor=>q[0] }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/RInit/App.pm]
#     test => { TESTS=>q[t/*.t] }
...
MAN3SECTION = 3
INSTALLDIRS = site
INSTALL_BASE = /home/liubianshi/perl5
DESTDIR = 
PREFIX = $(INSTALL_BASE)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/RInit/App.pm \
	lib/RInit/Manifest.pm
...
TO_INST_PM = lib/RInit/App.pm \
	lib/RInit/Manifest.pm \
	lib/RInit/Templates.pm
...
manifypods : pure_all config  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3SECTION) --perm_rw=$(PERM_RW) -u \
	  lib/RInit/App.pm $(INST_MAN3DIR)/RInit::App.$(MAN3EXT) \
	  lib/RInit/Manifest.pm $(INST_MAN3DIR)/RInit::Manifest.$(MAN3EXT) 
...
$(INST_SCRIPT)/rinit : bin/rinit $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/rinit
	$(CP) bin/rinit $(INST_SCRIPT)/rinit
	$(FIXIN) $(INST_SCRIPT)/rinit
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/rinit
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''A modern R project scaffolding tool written in Perl.'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: RInit' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   "abstract" : "A modern R project scaffolding tool written in Perl.",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "RInit",' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="RInit" VERSION="0.1.0">' > RInit.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>A modern R project scaffolding tool written in Perl.</ABSTRACT>' >> RInit.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Your Name &lt;your.email@example.com&gt;</AUTHOR>' >> RInit.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> RInit.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Cwd::" />' >> RInit.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="File::Basename" />' >> RInit.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="File::Path" />' >> RInit.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="File::Spec" />' >> RInit.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Getopt::Long" />' >> RInit.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Pod::Usage" />' >> RInit.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Term::ANSIColor" />' >> RInit.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-thread-multi-5.42" />' >> RInit.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> RInit.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> RInit.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> RInit.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e '$$i=0; $$n=$$#ARGV; $$i++ until $$i > $$n or $$ARGV[$$i] eq q{--};' \
	  ...
	  'lib/RInit/App.pm' 'blib/lib/RInit/App.pm' \
	  'lib/RInit/Manifest.pm' 'blib/lib/RInit/Manifest.pm' \
	  'lib/RInit/Templates.pm' 'blib/lib/RInit/Templates.pm' 
